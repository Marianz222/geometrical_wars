...
public class SistemaApuntado : MonoBehaviour
{
    ...
    public EventoDefinirObjetivoApuntado alDefinirObjetivo;
    ...
    void Start()
    {
        controlador_jugador = gameObject.GetComponentInParent<Jugador>();
        ...
        arma_actual = FindObjectOfType<Arma>();
        if (dispersion_arma == null) { return; }
    ...
    public Vector3 DefinirCoordenadasDisparoJugador()
    {
        ...
        if (Physics.Raycast(rayo, out RaycastHit golpe_rayo, distanciaMaxima, mascaraFiltro))
        {
            alDefinirObjetivo.Invoke(golpe_rayo.point);
            return golpe_rayo.point;
        }
        return rayo.origin + rayo.direction * distanciaMaxima;
    }
